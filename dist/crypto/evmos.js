...
const cosmos_1 = require("../common/cosmos");
...
class Evmos {
    ...
    //     return s;
    // }
    static signTransaction(_fromAddress, _toAddress, _amount, _denom, _memo, _fee, _gasLimit, _algo, _priv, _sequence, _accountNumber, _chainId) {
        // let pubKeyByte = Cosmos.getPubKeyAny(_priv);
        let pubKeyByte = cosmos_1.Cosmos.getPubKey(_priv);
        const pubKey = Buffer.from(pubKeyByte).toString("base64");
        const msg = (0, proto_1.createMsgSend)(_fromAddress, _toAddress, _amount, _denom);
        const body = (0, proto_1.createBody)(msg, _memo);
        const info = (0, proto_1.createSignerInfo)(_algo, pubKeyByte, _sequence, 1);
        const fee = (0, proto_1.createFee)(_amount, _denom, _gasLimit);
        const authInfo = (0, proto_1.createAuthInfo)(info, fee);
        const signDocDirect = (0, proto_1.createSigDoc)(body.serializeBinary(), authInfo.serializeBinary(), _chainId, _accountNumber);
